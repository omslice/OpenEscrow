...
| Wallet and testnet errors | Provider errors could expose RPC URLs, request bodies, and internal diagnostic language directly in red error messages. | Wallet cancellation, network congestion, connection loss, wrong-network, insufficient-test-ETH, and contract-rejection states now map to concise recovery guidance; raw provider diagnostics stay out of the consumer interface. |
