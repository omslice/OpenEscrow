...
The command exercises nineteen credential-free controls in memory:
...
7. recipient-specific multi-tenant claim notices with cross-tenant and query-link rejection;
8. response notices derived from the exact saved tenant decision and a canonical dashboard link;
9. role-isolated, token-free privacy inventory;
10. ciphertext, key-material, and digest tamper rejection;
11. isolated D1/R2 restoration that rejects missing and mislabeled key backups, safely verifies
   legacy metadata, and recovers exact bytes with the approved keyring;
12. evidence-upload outage and safe retry;
13. incomplete encrypted R2 upload cleanup after a metadata failure;
14. incomplete encrypted IPFS unpinning after a metadata failure;
15. evidence-download outage and privacy-safe failure;
16. notification-provider outage and idempotent recovery;
17. Base Sepolia receipt/event spoof rejection;
18. original-landlord verification and isolation for legacy receipt recovery; and
19. bounded receipt verification during public-RPC rate limiting.
...
| --- | --- | --- |
| Lost or untrusted signed-in browser | Use **End record sessions & sign out** from the affected verified account; keep that account selected until the result is shown and confirm prior derived sessions fail. If the account changes in flight, server containment still completes but local cleanup and provider sign-out are skipped for the new identity. Confirm the newly selected workspace contains none of the prior account's proposals, records, archive state, tracked ids, or late completion messages | Claim this revokes Privy, Google, wallet-provider, or invitation sessions |
...
| Evidence upload/download or metadata outage | Preserve the file hash and failure time; retry only after storage recovery; confirm no phantom metadata/event or incomplete R2 object/IPFS pin exists. The application attempts compensating cleanup when storage succeeds before D1 fails, but provider cleanup remains best-effort | Re-upload repeatedly, assume cleanup succeeded without checking provider state, publish plaintext elsewhere, or substitute public IPFS |
...
| Compliance source changed/stale/unreachable | Keep proposal finalization fail-closed and review the official source/version. If the gate closes after broadcast but before the receipt is saved, preserve the hash and reconcile the private/onchain mismatch explicitly. | Disable the source gate, reuse an earlier preflight as a waiver, or invent a legal rule |
