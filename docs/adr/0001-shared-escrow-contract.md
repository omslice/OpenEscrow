...
## Consequences
- All agreements share one token balance in aggregate; the contract-wide invariant `USDC.balanceOf(contract) == Σ(T_i + Ld_i + locked_i)` (spec §9) becomes the load-bearing safety property and must be invariant-fuzz-tested, not just unit-tested per agreement.
