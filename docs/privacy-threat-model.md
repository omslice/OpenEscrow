...
| Cross-account proposal, archive, device-local tracked-id, or account-center state | Server-side email/role matching, account-scoped archive rows, a workspace remount on stable Privy-account changes, account-scoped tracked-id storage, and stale completion guards for discovery, archive, inventory, wallet-copy, and embedded-wallet setup operations | Separate landlord/tenant discovery and archive-isolation rehearsal plus client regressions for account switching during in-flight workspace and account-center operations |
