...
| Cross-account proposal, archive, device-local tracked-id, or account-center state | Server-side email/role matching, account-scoped archive rows, a workspace remount on stable Privy-account changes, account-scoped tracked-id storage, and stale completion guards for discovery, archive, inventory, wallet-copy, and embedded-wallet setup operations | Separate landlord/tenant discovery and archive-isolation rehearsal plus a rendered two-identity switch regression that holds archive, inventory, and wallet operations in flight and proves no old data, download, or completion feedback reaches the new account |
