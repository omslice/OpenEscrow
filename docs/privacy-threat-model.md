...
| Cross-account proposal, archive, or device-local tracked-id access | Server-side email/role matching, account-scoped archive rows, a workspace remount on stable Privy-account changes, account-scoped tracked-id storage, and stale discovery/archive completion guards | Separate landlord/tenant discovery and archive-isolation rehearsal plus client regressions for account switching during in-flight discovery and archive operations |
