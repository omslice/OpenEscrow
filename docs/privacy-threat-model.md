...
| --- | --- | --- |
| Forged, expired, future-dated, overly long-lived, or wrong-application identity token | ES256 signature, issuer, audience, expiry, subject, linked-email, issued-at, and not-before verification. Privy keys must be P-256 signing keys, are fetched with a bounded timeout/response size, and are cached with shared in-flight requests and a guarded unknown-key refresh. | Server tests reject invalid tokens without creating an account session and prove repeated account operations reuse one validated key set. |
| Oversized or malformed API request exhausts Worker resources | Route-specific body ceilings are enforced from both declared length and the request stream before route parsing. Evidence keeps a separate upload allowance; malformed multipart data returns a consumer-safe validation error. | Server regressions reject declared and streamed oversized bodies and malformed multipart uploads before application work succeeds. |
| Repeated hosted API requests exhaust D1/R2, source checks, notification delivery, or identity verification | Hosted requests use endpoint-class limits stored atomically in D1. The limiter hashes the Cloudflare-provided client address, never reads a caller-provided bearer to choose the subject, expires old windows daily, and fails closed when the limit store is unavailable. | Server regressions exhaust the strict compliance-refresh bucket, prove changing an unverified authorization value cannot evade it, verify another client and later window remain usable, prove no raw address is stored, and simulate a rate-limit storage outage. |
| Unexpected server failure leaks a bearer, query credential, or internal error | The Worker returns a generic response with a request correlation ID and logs only the method, URL path, ID, and error class. Query strings, headers, bodies, tokens, and exception messages are excluded. | A forced database failure includes a sentinel secret in the query, authorization header, and exception; the regression proves neither response nor log contains it. |
...
| Agreement bearer leaks through a routine private-read URL | The client and server require `Authorization: Bearer` for record, report, and snapshot reads. Report download uses an authenticated fetch followed by a local browser download, so its URL contains only the non-secret download flag. Missing, malformed, or wrong headers fail closed; query-string credentials are ignored. Invitation entry still captures its one-time URL capability into same-tab recovery and scrubs the browser URL before these API reads. | Server regressions cover all three header-authorized reads, reject query-only credentials and downgrade attempts, and prove a valid header is authoritative over a bad query value. Client and rendered-browser regressions assert the bearer never appears in a private-read request URL and the complete report still downloads. |
...
| A wallet preflight is reused after an official source changes or becomes stale | Preflight records readiness for the audit trail but grants no finalization exception; receipt finalization rechecks the exact versioned source set and fails closed on every later blocking state | Server regression passes preflight, changes a required source, and proves finalization remains blocked without recording the agreement |
| A stored compliance snapshot has a forged collection, source, fact, cap, overlay, or claim-policy shape | Structural v3/v4 validation rejects the snapshot before evaluation; the workspace and report show an integrity warning without substituting current registry rules, and valid parsed evaluations are recursively detached and frozen | Shared/server regressions reject malformed stored shapes without throwing and prove later mutations cannot alter a completed evaluation |
...
| Duplicate, reorganized, unknown, or misattributed Base Sepolia activity | The hosted indexer reads only the configured OpenEscrow contract, waits 20 confirmations, validates exact log shape, stores a chain/transaction/log-index key once, and reconciles only an onchain agreement ID that uniquely matches one finalized D1 record. It never infers an email-to-wallet association; unknown events remain unmatched and deep reorganizations beyond the confirmation boundary are an explicitly monitored residual risk. | Server regressions reject malformed and wrong-contract logs, reconcile a confirmed direct withdrawal once, send only to opted-in recorded parties, and prove a second run creates neither a duplicate timeline item nor another provider request. |
| A participant tampers with automatic claim or response email copy | Claim notices are derived from the latest saved claim event and each review credential is exact-bound to its tenant. Response notices require the exact saved transaction for that tenant and derive the decision, note, amount, agreement ID, and canonical landlord dashboard link from server-controlled records. | Incident regressions reject missing/cross-tenant/query links and an unrecorded response receipt, ignore injected client copy, verify the canonical agreement link, and preserve delivery idempotency. |
...
| Misleading lifecycle record, malformed RPC result, or a real receipt relabeled as another party/value | Role/state guards, idempotency, bounded JSON-RPC responses with matching version/request ID, exact receipt transaction/block hashes and confirmed shape, custom-endpoint Base Sepolia chain verification, and one-log matching across deployment, event, agreement, exact participant, amount/hash/type fields; identical concurrent RPC work is shared. Finalization additionally proves every tenant/share, terms, and selected token at the confirmed block. | Credential-free lifecycle rehearsals plus spoofing regressions for wrong RPC IDs/chains/transaction hashes, oversized responses, simultaneous retries, wrong/missing/split finalization fields, reserve contract/escrow/tenant/token/share fields, another tenant's funding, aggregate-event substitution, claim/amendment values, tenant-response counts, ruling allocations, withdrawal parties/amounts, timeout outcomes, and altered registry hashes/types/actors. |
| Sensitive email content | Optional activity and deadline messages omit addresses, amounts, evidence, and notes. Direct claim and response notices go only to the recorded agreement parties, use saved event content, describe private evidence without embedding it, and give each tenant a separate private review link. | Scheduled-notification privacy checks plus claim/response recipient, stored-event, private-link, and idempotency regressions |
